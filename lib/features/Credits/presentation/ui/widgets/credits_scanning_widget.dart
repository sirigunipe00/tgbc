...
class CreditsCompScanningWidget extends StatefulWidget {
  const CreditsCompScanningWidget({
    super.key,
  ...
  final CreditsForm component;
  final void Function(String scanId) onScan;
  final void Function(File file) onAttachment;

  @override
  State<CreditsCompScanningWidget> createState() =>
      _CreditsCompScanningWidgetState();
}

class _CreditsCompScanningWidgetState extends State<CreditsCompScanningWidget> {
  String? scanVal;
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CreditsvalidationCubit, CreditsvalidationState>(
      listener: (_, state) {
        state.maybeWhen(
          orElse: () {},
      ...
      builder: (_, state) {
        return ListTile(
          ...
          subtitleTextStyle: Theme.of(context).textTheme.bodyLarge?.copyWith(fontSize: 12),
