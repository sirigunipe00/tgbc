...
import 'package:tgbc_app/features/unit_assemble-1/presentation/ui/widgets/unit1_scanning_widget.dart';
...
class UnitAssemblyScreen extends StatefulWidget {
  final String item;
  final int docst;
  const UnitAssemblyScreen(
      ...
      builder: (context) {
        return MultiBlocProvider(
          providers: [
            BlocProvider(
              create: (context) =>
                  Unit1BlocProvider.get().fetchUnit1ItemLines()..request(itemm),
            ),
            BlocProvider(
              create: (context) => Unit1BlocProvider.get().unitValidation(),
            ),
            BlocProvider(
              create: (context) => Unit1BlocProvider.get().unit1Submission(),
            ),
          ],
          child: UnitAssemblyScreen(item: itemm, docst: docsta),
        );
      },
    )).then(
      (value) {
  ...
  @override
  State<UnitAssemblyScreen> createState() => _UnitAssemblyScreenState();
}

class _UnitAssemblyScreenState extends State<UnitAssemblyScreen> {
  final lineItems = <UnitassemblyForm>[];
  List<UnitassemblyForm> completedLines = [];
  List<UnitassemblyForm> validScanItems = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: BlocConsumer<GetUnit1ItemsCubit, GetUnit1ItemsState>(
            ...
            state.maybeWhen(
              ...
              success: (data) {
                ...
                if (lineItems.isEmpty && widget.docst != 1) {
                  context.bloc<SubmitUnit1Cubit>().request(widget.item);
                }
              },
            );
          },
            ...
            return Column(
              ...
              children: [
                if (lineItems.isEmpty) ...[
                  Expanded(
                    child:
                        BlocListener<SubmitUnit1Cubit, SubmitUnit1CubitState>(
                      listener: (context, state) {
                        state.maybeWhen(
                            orElse: () => false,
                    ...
                    child: Expanded(
                      child: ListView.separated(
                        ...
                        itemBuilder: (ctxt, index) {
                          ...
                          return BlocProvider(
                            create: (_) =>
                                Unit1BlocProvider.get().unitValidation(),
