...
import 'package:tgbc_app/features/puf/presentation/ui/widgets/comp_scanning_widget.dart';
...
class _PufScreenState extends State<PufScreen> {
  final lineItems = <PufForm>[];
  List<PufForm> completedLines = [];
  List<PufForm> validScanItems = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: BlocConsumer<GetPufItemsCubit, GetPufItemsState>(
            ...
            state.maybeWhen(
              ...
              success: (data) {
                ...
                if (lineItems.isEmpty && widget.docst != 1) {
                  context.bloc<SubmitPufCubit>().request(widget.item);
                }
              },
            );
          },
            ...
            return Column(
              ...
              children: [
                if (lineItems.isEmpty) ...[
                  ...
                  BlocListener<SubmitPufCubit, SubmitPufCubitState>(
                    listener: (context, st) {
                      st.maybeWhen(
                        orElse: () => false,
                        failure: (failure) =>
                    ...
                    child: Expanded(
                      child: ListView.separated(
                        ...
                        itemBuilder: (ctxt, index) {
                          ...
                          return BlocProvider(
                            create: (_) =>
                                PufBlocProvider.get().scanningValidation(),
                            child: CompScanningWidget(
                              component: component,
                              docName: widget.item,
                              onAttachment: (file) {
                                setState(() {
